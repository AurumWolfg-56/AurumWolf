...
import { BusinessEntitySchema, BusinessMetricSchema, MetricSnapshotSchema } from '../lib/validators';
...
export function BusinessProvider({ children }: { children: React.ReactNode }) {
    ...
    const fetchUserData = async () => {
        ...
        try {
            ...
            const validEntities = parseSupabaseData(BusinessEntitySchema, entData, []) as BusinessEntity[];
            setEntities(validEntities);

            // Configs
            if (validEntities.length > 0) {
                const ids = validEntities.map(e => e.id);
                const { data: metData } = await supabase.from('business_metrics').select('*').in('business_id', ids);

                const validMetrics = parseSupabaseData(BusinessMetricSchema, metData, []) as BusinessMetric[];

                // Group by business
                const groupedMetrics: Record<string, BusinessMetric[]> = {};
                validMetrics.forEach((m: BusinessMetric) => {
                ...
                const validSnapshots = parseSupabaseData(MetricSnapshotSchema, snapData, []) as MetricSnapshot[];

                const groupedSnaps: Record<string, MetricSnapshot[]> = {};
                validSnapshots.forEach((s: MetricSnapshot) => {
    ...
    const addEntity = async (entity: BusinessEntity, packId?: string) => {
        ...
        try {
            const { data, error } = await supabase
                ...
                .insert([{
                    // Use the ID from the entity if provided (likely generated by client)
                    id: entity.id,
                    user_id: user.id,
                    name: entity.name,
                    type: entity.type,
            ...
            // We don't need to refresh generic config since we optimistically updated the entity list
            // But we might need to fetch the new metrics if a pack was applied.
            if (packId) {
                await fetchUserData();
            }
            ...
            setError(err.message);
            setEntities(prevEntities); // Rollback
        }
    };
    ...
    const updateEntity = async (entity: BusinessEntity) => {
        ...
        try {
            ...
            if (entity.customMetricsConfig) {
                ...
                // Fetch to sync detailed metrics state strictly
                await fetchUserData();
            }
    ...
    const deleteEntity = async (id: string) => {
        ...
        try {
            ...
            setLatestSnapshots(prevSnaps);
            setHealthScores(prevHealth);
        }
    };
