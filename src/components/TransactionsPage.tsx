...
import {
    ...
    LayoutList, ChevronLeft, ChevronRight, X, Plus, ScanLine, Search, Download, CheckSquare, Square, SlidersHorizontal,
    Repeat, Briefcase, ChevronDown, StopCircle, Trash2
} from 'lucide-react';
...
export const TransactionsPage: React.FC<TransactionsPageProps> = ({
    ...
    return (
        <div className="animate-fade-in space-y-6 pb-24 md:pb-0 relative">
                    ...
                    {filteredData.map(tx => (
                        <div key={tx.id} className="glass-card rounded-2xl p-5 relative group hover:border-gold-500/30 transition-all shadow-sm">
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-neutral-500 uppercase tracking-wider">{tx.recurringFrequency || t('transactions.unknown')} {t('transactions.cycle')}</p>
                                    </div>
                                </div>
                            ...
                            <div className="flex gap-2">
                                ...
                                <button
                                    onClick={(e) => handleStopSubscription(e, tx.id)}
            ...
            {isFilterOpen && (
                <div className="fixed inset-0 z-50 flex justify-end">
                    <div className="absolute inset-0 bg-neutral-950/60 backdrop-blur-sm" onClick={() => setIsFilterOpen(false)}></div>
                    <div className="relative w-full max-w-sm bg-white dark:bg-neutral-900 h-full shadow-2xl border-l border-platinum-200 dark:border-neutral-800 flex flex-col animate-fade-in">
                        ...
                        <div className="p-5 border-b border-platinum-200 dark:border-neutral-800 flex justify-between items-center bg-platinum-50 dark:bg-neutral-950">
                            ...
                            <button onClick={() => setIsFilterOpen(false)} className="text-neutral-500 hover:text-neutral-900 dark:hover:text-white">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-8 custom-scrollbar">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input type="date" value={dateRange.start} onChange={(e) => setDateRange({ ...dateRange, start: e.target.value })} className="w-full bg-platinum-50 dark:bg-neutral-950 border border-platinum-200 dark:border-neutral-800 rounded-lg p-2 text-xs text-neutral-900 dark:text-white outline-none focus:border-gold-500 [&::-webkit-calendar-picker-indicator]:invert dark:[&::-webkit-calendar-picker-indicator]:invert-0" />
                                    </div>
                                    <div>
                                        <label className="text-[10px] text-neutral-600 block mb-1">{t('common.to')}</label>
                                        <input type="date" value={dateRange.end} onChange={(e) => setDateRange({ ...dateRange, end: e.target.value })} className="w-full bg-platinum-50 dark:bg-neutral-950 border border-platinum-200 dark:border-neutral-800 rounded-lg p-2 text-xs text-neutral-900 dark:text-white outline-none focus:border-gold-500 [&::-webkit-calendar-picker-indicator]:invert dark:[&::-webkit-calendar-picker-indicator]:invert-0" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center gap-3">
                                    <input type="number" placeholder="Min" value={amountRange.min} onChange={(e) => setAmountRange({ ...amountRange, min: e.target.value })} className="w-full bg-platinum-50 dark:bg-neutral-950 border border-platinum-200 dark:border-neutral-800 rounded-lg p-2 text-xs text-neutral-900 dark:text-white outline-none focus:border-gold-500" />
                                    <span className="text-neutral-600">-</span>
                                    <input type="number" placeholder="Max" value={amountRange.max} onChange={(e) => setAmountRange({ ...amountRange, max: e.target.value })} className="w-full bg-platinum-50 dark:bg-neutral-950 border border-platinum-200 dark:border-neutral-800 rounded-lg p-2 text-xs text-neutral-900 dark:text-white outline-none focus:border-gold-500" />
                                </div>
                            </div>
                        ...
                        <div className="p-5 border-t border-platinum-200 dark:border-neutral-800 bg-platinum-50 dark:bg-neutral-950 flex gap-3">
                            <button onClick={clearFilters} className="flex-1 py-3 rounded-xl border border-platinum-300 dark:border-neutral-800 text-neutral-500 font-bold hover:bg-white dark:hover:bg-neutral-900 transition-colors">
                                {t('common.reset')}
                            </button>
                            <button onClick={() => setIsFilterOpen(false)} className="flex-1 py-3 rounded-xl bg-gold-500 text-neutral-950 font-bold hover:bg-gold-400 transition-colors shadow-lg">
                                {t('common.apply')}
                            </button>
                        </div>
